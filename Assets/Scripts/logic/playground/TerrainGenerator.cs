	...
	public class TerrainGenerator : MonoBehaviour, ISettingsDebugRefreshListener
	{
		...
		private void DrawTerrain()
		{
			...
			for (int y = 0; y < terrainSize.y; y++)
			{
				...
				{
					var blockInstance = Instantiate(playgroundSettings.terrainTexturePrefab, gameObject.transform, true);
					blockInstance.name = $"{playgroundSettings.terrainTexturePrefab.name}[{x},{y}]";
