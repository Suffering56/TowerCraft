...
namespace logic.playground
{
	...
	{
		[SerializeField]
		private GameObject gridTexturePrefab;
		...
		{
			var blockInstance = Instantiate(gridTexturePrefab, gameObject.transform, true);
			blockInstance.name = $"{gridTexturePrefab.name}[{xName},{yName}]";
